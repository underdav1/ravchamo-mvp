 ...
 *   randomness?: number // optional score jitter, 0..1 (default 0.35 server-side)
 * }
 *
 * Returns top 10 dishes from the Supabase recommend_dishes RPC.
 * Adds two sources of variety on top of the existing scoring:
 *   - server-side multiplicative score jitter (passed via `randomness`)
 *   - session-level exclude list of recently shown dish ids (localStorage)
 */
export async function recommend(user) {
  ...
  // Postgres-side default (0.35) keeps applying for normal searches.
